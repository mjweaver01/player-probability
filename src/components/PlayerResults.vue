  ...
  <div class="transition-all duration-300">
    <div class="bg-white rounded-xl overflow-hidden shadow-lg">
      <div class="bg-gradient-to-r from-indigo-600 to-blue-500 p-6 text-white">
        <h2 class="text-2xl font-bold flex align-center justify-between">
          {{ result.player || playerName }}<span class="text-right" v-html="parsedTeam"></span>
        </h2>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const parsedTeam = computed(() => (props.result.team ? marked.parse(props.result.team) : ''));
